...
write_account() {
...

  {
    ...
    printf '"/usr/bin/security", "find-generic-password", "-w", "-s", %s, "-a", %s' \
...
configure() {
  ...
  write_account "$temporary" "${accounts[0]}" "${emails[0]}" true
  write_account "$temporary" "${accounts[1]}" "${emails[1]}" false
