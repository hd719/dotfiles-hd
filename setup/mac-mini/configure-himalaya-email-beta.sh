...
check_config() {
  ...
  (( ${#accounts[@]} >= 1 && ${#accounts[@]} <= 2 )) \
    || die "expected one or two accounts, found ${#accounts[@]}"
...
configure() {
  ...
  read -r -p 'Number of accounts to configure [1-2]: ' account_count
  [[ "$account_count" =~ ^[12]$ ]] || die 'account count must be 1 or 2'
  ...
  trap 'rm -f "${temporary:-}"' EXIT
  {
    ...
    printf '# IMAP-only one-or-two-account beta. SMTP is intentionally absent.\n\n'
  } > "$temporary"
