...
link_real_commands() {
  ...
  for name in \
    awk basename cat chmod cmp cp cut date dirname env grep head ln mkdir mktemp mv \
...
make_fake_machine() {
  ...
  # npm records global installs and materializes the command names supplied by
  # each pinned package. The prefix remains private to the fake HOME.
  write_stub "$NPM_STUB" \
    ...
    '    cp "$TEST_GENERIC_STUB" "$prefix/bin/$command_name"' \
  ...
  write_stub "$SYSTEM_BIN/apt-command" \
    ...
    '    npm) cp "$TEST_NPM_STUB" "$TEST_LOCAL_BIN/npm"; chmod +x "$TEST_LOCAL_BIN/npm"; continue ;;' \
...
seed_host_runtimes() {
  ...
  cp "$NPM_STUB" "$LOCAL_BIN/npm"
  cp "$GO_STUB" "$LOCAL_BIN/go"
  install_generic_tool gofmt
  chmod +x "$LOCAL_BIN/npm" "$LOCAL_BIN/go"
}

seed_supported_node_and_npm() {
  install_generic_tool node
  cp "$NPM_STUB" "$LOCAL_BIN/npm"
...
seed_mise_node_without_npm() {
  MISE_NODE_BIN="$FAKE_HOME/.local/share/mise/installs/node/22.22.0/bin"
  mkdir -p "$MISE_NODE_BIN"
  cp "$GENERIC_STUB" "$MISE_NODE_BIN/node"
  chmod +x "$MISE_NODE_BIN/node"

  # Model the repair operation used by a real mise-managed Node installation:
  # reinstall the exact active version, restore npm beside it, then reshim.
  write_stub "$LOCAL_BIN/mise" \
    'printf "mise %s\n" "$*" >>"$TEST_TRACE_LOG"' \
    'case "${1:-}" in' \
    '  which)' \
    '    [[ "${2:-}" == "node" ]] || exit 1' \
    ...
    '    ;;' \
    '  install)' \
    '    [[ " $* " == *" --force node@22.22.0 "* ]] || exit 91' \
    '    printf "mise-install %s\n" "$*" >>"$TEST_MUTATION_LOG"' \
    '    cp "$TEST_NPM_STUB" "$TEST_MISE_NODE_BIN/npm"' \
    '    chmod +x "$TEST_MISE_NODE_BIN/npm"' \
    '    ;;' \
    '  reshim)' \
    '    printf "mise-reshim\n" >>"$TEST_MUTATION_LOG"' \
    '    ;;' \
  ...
  export TEST_MISE_NODE_BIN="$MISE_NODE_BIN"
  TEST_PATH="$MISE_NODE_BIN:$LOCAL_BIN:$SYSTEM_BIN"
}

assert_common_safety() {
  expect_no_log "Homebrew and Snap are never invoked" '^FORBIDDEN ' "$TRACE_LOG"
  expect_no_log \
    "npm, npx, and npm's package directory are never removed" \
...
assert_core_architecture() {
  ...
  expect_no_log "core excludes npm language servers" '^npm-install '
...
# Core never needs Node-backed language servers. Even if it sees the legacy
# "mise Node exists, npm is missing" state, it must leave that runtime alone.
make_fake_machine "core-mise-without-npm" arm64
seed_core_ready
seed_mise_node_without_npm
run_installer core
expect_success "core ignores missing npm in a mise Node"
expect_no_log "core never invokes mise" '^mise ' "$TRACE_LOG"
expect "core leaves npm absent" \
  bash -c 'PATH="$1"; export PATH; ! command -v npm >/dev/null 2>&1' _ "$TEST_PATH"
expect "ready core performs no mutations" test ! -s "$MUTATION_LOG"
assert_common_safety
...
# A known-unsupported host runtime must stop before APT, downloads, npm, or Go
...
for command_name in \
  ...
  vscode-css-language-server vscode-html-language-server graphql-lsp
do
  expect "missing $command_name receives a local link" test -L "$LOCAL_BIN/$command_name"
done
assert_common_safety

# Full adds pinned user-local tools without changing a working Node/npm/Go host
...
cp "$LOCAL_BIN/npm" "$STATE_DIR/npm.before"
run_installer full
...
expect_log "bash-language-server is version-pinned" 'npm-install .*bash-language-server@[0-9]'
expect_log "vtsls is version-pinned" 'npm-install .*@vtsls/language-server@[0-9]'
expect_log "VSCode language servers are version-pinned" 'npm-install .*vscode-langservers-extracted@[0-9]'
expect_log "GraphQL language server is version-pinned" 'npm-install .*graphql-language-service-cli@[0-9]'
...
expect "host npm is byte-identical" cmp -s "$STATE_DIR/npm.before" "$LOCAL_BIN/npm"
expect "host npm remains a regular command" test ! -L "$LOCAL_BIN/npm"
expect "host Node version is unchanged" \
  bash -c 'PATH="$1"; export PATH; test "$(node --version)" = "v22.22.0"' _ "$TEST_PATH"
expect "host npm version is unchanged" \
  bash -c 'PATH="$1"; export PATH; test "$(npm --version)" = "10.9.4"' _ "$TEST_PATH"
for command_name in \
...
expect "second full run preserves npm" cmp -s "$STATE_DIR/npm.before" "$LOCAL_BIN/npm"
assert_common_safety
...
# A prior Ubuntu revision could leave mise's active Node intact while deleting
# npm. Repair must reinstall that exact Node version in place, restore npm, and
# become a no-op once the capability is healthy.
make_fake_machine "full-mise-npm-repair" arm64
seed_core_ready
cp "$GO_STUB" "$LOCAL_BIN/go"
install_generic_tool gofmt
chmod +x "$LOCAL_BIN/go"
seed_mise_node_without_npm
cp "$MISE_NODE_BIN/node" "$STATE_DIR/mise-node.before"
expect "mise scenario begins without npm" \
  bash -c 'PATH="$1"; export PATH; ! command -v npm >/dev/null 2>&1' _ "$TEST_PATH"

run_installer full
expect_success "full repairs missing mise npm"
expect_log "mise repair pins the active Node version" '^mise-install install --force node@22\.22\.0$'
expect "mise Node remains byte-identical" cmp -s "$STATE_DIR/mise-node.before" "$MISE_NODE_BIN/node"
expect "mise Node remains at the original path" \
  bash -c 'PATH="$1"; export PATH; test "$(command -v node)" = "$2/node"' _ "$TEST_PATH" "$MISE_NODE_BIN"
expect "mise Node remains at the original version" \
  bash -c 'PATH="$1"; export PATH; test "$(node --version)" = "v22.22.0"' _ "$TEST_PATH"
expect "npm is restored beside the active mise Node" test -x "$MISE_NODE_BIN/npm"
expect "the restored mise npm wins on PATH" \
  bash -c 'PATH="$1"; export PATH; test "$(command -v npm)" = "$2/npm"' _ "$TEST_PATH" "$MISE_NODE_BIN"
expect "the restored mise npm is usable" \
  bash -c 'PATH="$1"; export PATH; test "$(npm --version)" = "10.9.4"' _ "$TEST_PATH"
expect_no_log "APT does not replace repaired Node or npm" 'apt-install .*(nodejs|npm)'
assert_common_safety

cp "$MUTATION_LOG" "$STATE_DIR/mise-repair-first-run.log"
run_installer full
expect_success "second repaired-mise full run succeeds"
expect "mise npm repair is idempotent" \
  cmp -s "$STATE_DIR/mise-repair-first-run.log" "$MUTATION_LOG"
expect "second run keeps the same mise Node bytes" \
  cmp -s "$STATE_DIR/mise-node.before" "$MISE_NODE_BIN/node"
expect "second run keeps npm beside the same mise Node" test -x "$MISE_NODE_BIN/npm"
assert_common_safety

# Forced mise repair replaces the whole Node version directory. Refuse that
# shortcut when it would endanger any unrelated global package or executable.
make_fake_machine "mise-repair-with-global-package" arm64
seed_core_ready
cp "$GO_STUB" "$LOCAL_BIN/go"
install_generic_tool gofmt
chmod +x "$LOCAL_BIN/go"
seed_mise_node_without_npm
MISE_NODE_ROOT="${MISE_NODE_BIN%/bin}"
mkdir -p "$MISE_NODE_ROOT/lib/node_modules/acme-tool"
printf '{"name":"acme-tool"}\n' >"$MISE_NODE_ROOT/lib/node_modules/acme-tool/package.json"
run_installer full
expect "mise repair refuses an unrelated global package" test "$RUN_STATUS" -ne 0
expect_output "global-package refusal names the package" 'acme-tool'
expect_output "global-package refusal explains npm repair" 'npm.*repair|repair.*npm'
expect "global-package refusal happens before mutations" test ! -s "$MUTATION_LOG"
expect "npm remains absent after package refusal" test ! -e "$MISE_NODE_BIN/npm"
assert_common_safety

make_fake_machine "mise-repair-with-global-binary" arm64
seed_core_ready
cp "$GO_STUB" "$LOCAL_BIN/go"
install_generic_tool gofmt
chmod +x "$LOCAL_BIN/go"
seed_mise_node_without_npm
cp "$GENERIC_STUB" "$MISE_NODE_BIN/acme-cli"
chmod +x "$MISE_NODE_BIN/acme-cli"
run_installer full
expect "mise repair refuses an unrelated global binary" test "$RUN_STATUS" -ne 0
expect_output "global-binary refusal names the command" 'acme-cli'
expect_output "global-binary refusal explains npm repair" 'npm.*repair|repair.*npm'
expect "global-binary refusal happens before mutations" test ! -s "$MUTATION_LOG"
expect "npm remains absent after binary refusal" test ! -e "$MISE_NODE_BIN/npm"
assert_common_safety
...
expect_no_log "gopls replacement does not reinstall unrelated tools" \
  '^(apt-install|download|npm-install) '
assert_common_safety
