...
account password. Do not enable notification automation until each configured
account passes `--check` and the read-only mailbox QA is green. The check fails
closed when Himalaya reports a backend authentication failure.
