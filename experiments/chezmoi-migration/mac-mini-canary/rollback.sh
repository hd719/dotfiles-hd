...
backup="$HOME/.local/state/chezmoi-mac-mini-canary/pre-apply.tar"
...
grep -Fxq '# pre-chezmoi Mac mini canary marker' "$HOME/.zshrc"
grep -Fxq 'runtime log' "$HOME/.config/btop/btop.log"
grep -Fxq 'legacy state' "$HOME/.config/fastfetch/legacy/sentinel"
grep -Fxq 'runtime state' "$HOME/.config/herdr/session"
grep -Fxq '{"state":"machine-owned"}' "$HOME/.config/hunk/state.json"
grep -Fxq 'machine state' "$HOME/.config/mise/state.toml"
grep -Fxq '{"session":"machine-owned"}' "$HOME/.hermes/session.json"
! grep -Fq "$HOME/.config/git/aliases.gitconfig" "$HOME/.gitconfig"
printf 'Rollback restored the pre-chezmoi Mac mini canary state.\n'
