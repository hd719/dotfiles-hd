...
backup="$(mktemp "${TMPDIR:-/tmp}/chezmoi-mac-mini-lazy-lock.XXXXXX")"
...
DOTFILES_NVIM_PROFILE=full DOTFILES_NVIM_RESTORE_ALL=1 \
  nvim --headless '+Lazy! restore' '+qa'
DOTFILES_NVIM_PROFILE=full nvim --headless \
  "+lua local ok=require('nvim-treesitter').install({'markdown','markdown_inline'}):wait(); if not ok then vim.cmd('cquit 1') end" \
  '+qa'

restore_lockfile
trap - EXIT
printf 'Full-profile Neovim restored without changing lazy-lock.json.\n'
