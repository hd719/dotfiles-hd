...
`hosts/mac-work/.zshrc` loads the full Mac development interface,
...
brew bundle install --no-upgrade \
  --file="$HOME/Developer/dotfiles-hd/hosts/mac-work/Brewfile"
...
(
  ...
  test "$(readlink "$HOME/.zshrc" 2>/dev/null || true)" != \
    "$HOME/Developer/dotfiles-hd/hosts/mac-work/.zshrc"
)
```
...
`hd-stop` frees the documented development ports, then uses Herdr's native
