...
# Refuse known-bad profile state before installers can make a partial
...
if [[ "$MODE" == "dry-run" ]]; then
  ...
  if [[ "$PROFILE" == mac-studio ]]; then
    say "Ubuntu stays dormant; provider setup is deferred until manual use"
...
if [[ "$PROFILE" == mac-mini ]]; then
  DOTFILES_CHEZMOI_APPROVED=1 DOTFILES_MAC_MINI_CONFIG_ONLY=1 \
    "$CHEZMOI_BOOTSTRAP" "$PROFILE" --apply
else
  DOTFILES_CHEZMOI_APPROVED=1 "$CHEZMOI_BOOTSTRAP" "$PROFILE" --apply
fi
