...
Bento Ubuntu 26.04 ARM64
  ...
  -> Chezmoi user configuration and mise tools
  -> doctor.sh --offline
```
...
```bash
cd ~/Developer/dotfiles-hd/hosts/ubuntu-dev
vagrant ssh -c \
...
- `setup-neovim.sh` maintains mise and Neovim only after verifying the required
  Chezmoi-managed links.
- `update-system.sh` performs later Ubuntu and tool updates.
- `doctor.sh` verifies the finished workstation.

The host-local readiness record is
...
```bash
bash hosts/ubuntu-dev/doctor.sh --offline
```
...
```bash
bash hosts/ubuntu-dev/update-system.sh
bash hosts/ubuntu-dev/doctor.sh
```
...
`herdr server reset`, aliased to `hdk`, closes every local Herdr workspace and
preserves one fresh `home` workspace rooted at `~`. From a regular terminal it
also stops the server, so the next `herdr` starts from that clean state. Run
from inside Herdr it leaves the server up and drops you in the fresh `home`
workspace, closing the workspace that owns the shell last. Add `--dry-run` to
preview it.
