...
NEOVIM_SCRIPT="$ROOT_DIR/hosts/ubuntu-dev/setup-neovim.sh"
UPDATE_SCRIPT="$ROOT_DIR/hosts/ubuntu-dev/update-system.sh"
DOCTOR_SCRIPT="$ROOT_DIR/hosts/ubuntu-dev/doctor.sh"
UBUNTU_README="$ROOT_DIR/hosts/ubuntu-dev/README.md"
UBUNTU_GUIDE="$ROOT_DIR/hosts/ubuntu-dev/GUIDE.md"
MISE_CONFIG="$ROOT_DIR/hosts/ubuntu-dev/mise.toml"
ZSH_CONFIG="$ROOT_DIR/hosts/ubuntu-dev/.zshrc"
GHOSTTY_CONFIG="$ROOT_DIR/hosts/ubuntu-dev/ghostty.conf"
GRAPHQL_WRAPPER="$ROOT_DIR/hosts/ubuntu-dev/bin/graphql-lsp"
CODEX_WRAPPER="$ROOT_DIR/hosts/ubuntu-dev/bin/codex"
GIT_ALIASES_SCRIPT="$ROOT_DIR/config/git/configure-aliases.sh"
GIT_ALIASES_CONFIG="$ROOT_DIR/config/git/aliases.gitconfig"
SSH_CONFIG="$ROOT_DIR/hosts/ubuntu-dev/ssh/config"
VAGRANTFILE="$ROOT_DIR/hosts/ubuntu-dev/Vagrantfile"
ANSIBLE_BOOTSTRAP="$ROOT_DIR/hosts/ubuntu-dev/bootstrap-ansible.sh"
ANSIBLE_DIR="$ROOT_DIR/hosts/ubuntu-dev/ansible"
...
seed_neovim_managed_links() {
  local home_dir="$1"

  mkdir -p \
    "$home_dir/.config/mise" \
    "$home_dir/.local/graphql-lsp/bin"
  ln -s "$MISE_CONFIG" "$home_dir/.config/mise/config.toml"
  ln -s "$ROOT_DIR/config/nvim" "$home_dir/.config/nvim"
  ln -s "$GRAPHQL_WRAPPER" "$home_dir/.local/graphql-lsp/bin/graphql-lsp"
}
...
test_vagrant_ansible_contract() {
  ...
  assert_file_contains "$ROOT_DIR/.gitignore" 'hosts/ubuntu-dev/.vagrant/'
...
test_doctor_is_read_only_and_complete() {
  ...
  local link_specs=(
    "hosts/ubuntu-dev/.zshrc|.zshrc"
    "hosts/ubuntu-dev/ghostty.conf|.config/ghostty/config"
    "config/starship/starship.toml|.config/starship.toml"
    "config/git/.gitignore_global|.gitignore_global"
    "hosts/ubuntu-dev/ssh/config|.ssh/config"
    ...
    "hosts/ubuntu-dev/mise.toml|.config/mise/config.toml"
    "config/nvim|.config/nvim"
    "config/tmux|.config/tmux"
    "hosts/ubuntu-dev/bin/codex|.local/bin/codex"
    "hosts/ubuntu-dev/bin/graphql-lsp|.local/graphql-lsp/bin/graphql-lsp"
  )
  ...
  for spec in "${link_specs[@]}"; do
    ...
    assert_file_contains "$UBUNTU_README" "\`${spec%%|*}\`"
    if [[ "$source" == */hosts/ubuntu-dev/ghostty.conf ]]; then
  ...
  assert_file_contains "$UBUNTU_README" 'bash hosts/ubuntu-dev/doctor.sh'
  assert_file_contains "$UBUNTU_GUIDE" 'bash hosts/ubuntu-dev/doctor.sh'
  assert_file_contains "$UBUNTU_GUIDE" 'bash hosts/ubuntu-dev/doctor.sh --offline'
...
[[ "${1:-}" == "--check" ]]
EOF
  cat > "$case_dir/chezmoi-doctor.sh" <<'EOF'
#!/usr/bin/env bash
[[ "${1:-}" == "ubuntu" ]] || exit 2
[[ "${DOTFILES_TEST_CHEZMOI_FAIL:-0}" != "1" ]]
EOF
  chmod +x \
    "$case_dir/bin/"* \
    "$case_dir/chezmoi-doctor.sh" \
    "$case_dir/neovim-check.sh"
...
test_obsolete_ubuntu_helpers_are_gone() {
  ...
  for file in \
    ...
    setup-ulauncher.sh \
    setup.sh; do
    [[ ! -e "$ROOT_DIR/hosts/ubuntu-dev/$file" ]] || fail "obsolete Ubuntu helper remains: $file"
  done
}
...
test_neovim_setup_installs_and_checks_daily_driver() {
  ...
  seed_neovim_managed_links "$case_dir/home"
...
EOF
  ...
  [[ -L "$case_dir/home/.config/mise/config.toml" ]] || fail "Chezmoi-managed mise config is missing"
  [[ "$(readlink "$case_dir/home/.config/mise/config.toml")" == "$MISE_CONFIG" ]] || fail "mise config points to the wrong source"
  [[ -d "$case_dir/home/.config/mise" && ! -L "$case_dir/home/.config/mise" ]] || fail "mise parent must remain a managed directory"
  [[ -L "$case_dir/home/.config/nvim" ]] || fail "Chezmoi-managed Neovim config is missing"
  [[ "$(readlink "$case_dir/home/.config/nvim")" == "$ROOT_DIR/config/nvim" ]] || fail "Neovim config points to the wrong source"
  [[ -L "$case_dir/home/.local/graphql-lsp/bin/graphql-lsp" ]] || fail "Chezmoi-managed GraphQL wrapper is missing"
  ...
  seed_neovim_managed_links "$fresh_home"
...
EOF
  ...
  seed_neovim_managed_links "$broken_home"
...
EOF
  ...
  seed_neovim_managed_links "$failed_home"
  cat > "$case_dir/bin/curl" <<'EOF'
#!/usr/bin/env bash
printf '#!/usr/bin/env bash\nexit 0\n'
EOF
  ...
  [[ -L "$failed_home/.config/nvim" \
    && "$(readlink "$failed_home/.config/nvim")" == "$ROOT_DIR/config/nvim" ]] \
    || fail "failed mise installation changed Chezmoi-managed Neovim configuration"
}
