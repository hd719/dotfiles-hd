...
run_personal_ready() {
  ...
  capture "$ZSH_BIN" -c "source '$DOTFILES_ROOT/hosts/thin-mac/vm.zsh'; uvm-status"
  ...
  if [[ "$vm_output" == *"running"* ]]; then
    ...
    capture "$ZSH_BIN" -c "source '$DOTFILES_ROOT/hosts/thin-mac/vm.zsh'; uvm-up"
    if ((LAST_STATUS == 0)); then
      capture "$ZSH_BIN" -c "source '$DOTFILES_ROOT/hosts/thin-mac/vm.zsh'; uvm-status"
    fi
  ...
  ssh_capture "$vm_host" 'cd /home/hamel/Developer/dotfiles-hd && bash hosts/ubuntu-dev/update-system.sh'
  if ((LAST_STATUS == 0)); then
    ...
    add_result FAIL ubuntu-vm "Ubuntu updater" \
      ...
      "Review the named stage, then rerun hosts/ubuntu-dev/update-system.sh"
  fi

  ssh_capture "$vm_host" 'cd /home/hamel/Developer/dotfiles-hd && bash hosts/ubuntu-dev/doctor.sh'
  record_command_result PASS FAIL ubuntu-vm "Ubuntu doctor" \
    "doctor passed" "doctor reported failures" \
    "cd /home/hamel/Developer/dotfiles-hd && bash hosts/ubuntu-dev/doctor.sh"
