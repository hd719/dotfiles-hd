#!/bin/sh
set -eu
profile="${1:-}"; mode="${2:---preview}"; version="v2.72.0"
script_dir="$(CDPATH= cd -- "$(dirname -- "$0")" && pwd -P)"
bin="${CHEZMOI_BIN:-$HOME/.local/bin/chezmoi}"
case "$profile:$mode" in
  ubuntu:--preview|ubuntu:--apply|mac-thin:--preview|mac-thin:--apply|mac-pro:--preview|mac-pro:--apply|mac-studio:--preview|mac-studio:--apply|mac-mini:--preview|mac-mini:--apply|mac-work:--preview|mac-work:--apply) ;;
  *) echo "usage: $0 ubuntu|mac-thin|mac-pro|mac-studio|mac-mini|mac-work --preview|--apply" >&2; exit 2 ;;
esac
...
if [ "$mode" = --apply ] || [ "${DOTFILES_CHEZMOI_REQUIRE_REVIEWED:-0}" = 1 ]; then
  bash "$script_dir/preflight.sh" "$profile"
fi
if [ ! -x "$bin" ]; then
  mkdir -p "$(dirname "$bin")"
